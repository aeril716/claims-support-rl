...
def score(args):
    ...
    print(f"judge {judge.judge_info()} | {len(items)} completions", flush=True)
    t0 = time.time()
...
def compare(args):
    ...
    print(f"{len(common)} completions, {tot['n']} item verdicts | rubric mean {args.label_a} "
          f"{sum(ra)/len(ra):.3f} vs {args.label_b} {sum(rb)/len(rb):.3f} | overall agreement {tot['agree']/tot['n']:.3f}\n")
